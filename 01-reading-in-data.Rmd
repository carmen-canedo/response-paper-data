...
# Structure
I am reading in the data for the United States and Mexico from Box file.

## Libraries
Bringing in the same libraries used in lapop-dashboard repo.
```{r}
library(labelled)
library(sjmisc)
library(tidyverse)
...
```{r}
usa_factor <- factor_country("data/merged_bolivia_2004_2014.zip")
usa_unfactor <- read_dta("data/merged_bolivia_2004_2014.zip")
mexico_factor <- factor_country("data/merged_bolivia_2004_2014.zip")
mexico_unfactor <- read_dta("data/merged_bolivia_2004_2014.zip")
```
...
### USA 2016 File
```{r}
usa_16_factor  <- factor16_country("data/2016_bolivia.dta") %>% 
  #Checking for uniqueness
  assert(is_uniq, person_id_factor)

usa_16_unfactor  <- read_dta("data/2016_bolivia.dta") %>% 
  #Renaming ID column
  rename(person_id = uniq_id) %>% 
  #Checking for uniqueness
...
### Mexico 2016 File
```{r}
mexico_16_factor  <- factor16_country("data/2016_bolivia.dta") %>% 
  #Checking for uniqueness
  assert(is_uniq, person_id_factor)

mexico_16_unfactor  <- read_dta("data/2016_bolivia.dta") %>% 
  #Renaming ID column
  rename(person_id = uniq_id) %>% 
  #Checking for uniqueness
  assert(is_uniq, person_id) %>% 
...
### USA 2018 file
```{r}
usa_18_factor <- factor18_country("data/2018_bolivia.dta") %>% 
  #Checking for uniqueness
  assert(is_uniq, person_id_factor)

usa_18_unfactor <- read_dta("data/2018_bolivia.dta") %>%
  #Renaming ID column
  rename(person_id = uniq_id) %>% 
...
### Mexico 2018 file
```{r}
mexico_18_factor <- factor18_country("data/2018_bolivia.dta") %>% 
  #Checking for uniqueness
  assert(is_uniq, person_id_factor)

mexico_18_unfactor <- read_dta("data/2018_bolivia.dta") %>%
  #Renaming ID column
  rename(person_id = uniq_id) %>% 
  #Checking for uniquness
  assert(is_uniq, person_id) %>% 
